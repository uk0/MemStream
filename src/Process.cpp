...
namespace memstream {
    ...
    void Process::StageRead(uint64_t addr, uint8_t *buffer, uint32_t size) {
        ScatterOp op(addr, buffer, size);
        if(!op.Valid()) return;

        this->stagedReads.push_front(op);
    }
    ...
    bool Process::ExecuteStagedReads() {
        ...
        while(!this->stagedReads.empty()) {
            ...
            // any failed
            this->stagedReads.remove_if([](const ScatterOp& op){ 
    ...
    bool Process::ReadMany(std::list<ScatterOp> &operations) {
        ...
        std::list<ScatterOp>::iterator it;
    ...
    void Process::StageWrite(uint64_t addr, uint8_t *buffer, uint32_t size) {
        ...
        this->stagedWrites.push_front(op);
    }
    ...
    bool Process::WriteMany(std::list<ScatterOp>& operations) {
        // reinit VMM scatter
        if(!this->scatter) {
            this->scatter = VMMDLL_Scatter_Initialize(
                    this->pFPGA->getVmm(),
